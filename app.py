...
from sensor_api import generate_sensor_data,get_patient_data,getPrediction
...
st.divider()
st.header("Check Risks using ML")

heart_rate = st.number_input("enter heart rate",key=1)
systolic_bp = st.number_input("enter systolic_bp ",key=2)
diastolic_bp = st.number_input("enter diastolic_bp ",key=3)
body_temp = st.number_input("enter body_temp",key=4)


if st.button("Get Prediction"):
    ans=getPrediction(heart_rate,systolic_bp,diastolic_bp,body_temp)[0]
    st.write(f"Your health status : {ans}")
else:
    st.write("get Prediction Here")


# 60.91975924478789,129.5312704531384,75.48841141591001,36.83982634590284,